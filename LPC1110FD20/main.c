...
{
    LPC_IOCON->JTAG_TMS_PIO1_0 = 2;                       // AD1/P1.0 = AD1 ~~~~~~~~~ set sensor as AD1 input
    LPC_ADC->CR = 2 |                              // SEL = 2, select channel 1 on ADC
                 (11 << 8) |                        // ADC_CLK = Fpclk / CLKDIV = 48 MHz/12 = 4 MHz
                 (1 << 24);                        // start conversion

    while (!(LPC_ADC->DR[1] & ADC_DONE)) ;         // wait until end of AD1 conversion

    LPC_IOCON->JTAG_TMS_PIO1_0 = 0x81;                    // AD1/P0.1 = GPO ~~~~~~~~~ charge the cap plate
...
int main (void)
{
  ...
  static char  ledon  = 0;
    ...
    LPC_GPIO0->DATA = (1<<1) | (1<<2); 
    LPC_GPIO1->DIR |= (1<<0);                      // P1.0 = output connected to cap sense plate
    LPC_GPIO0->DIR |= (1<<1);                      // P0.1 = toggle LED
    LPC_GPIO0->DIR |= (1<<2);                      // P0.2 = ON/OFF LED
    ...
    while (1)
    {
        ...
        {
            LPC_GPIO0->DATA &= ~(1<<2);            // P0.2 low = LED ON
            if (!toggle)
            {
                toggle = 1;
                if (!ledon)
                {
                    ledon = 1;
                    LPC_GPIO0->DATA &= ~(1<<1);    // P0.1 low = LED ON
                }
                else
                {
                    ledon = 0;
                    LPC_GPIO0->DATA |= (1<<1);     // P0.1 high = LED OFF
                }
            }
        }
        else                                       // key released
        {
            LPC_GPIO0->DATA |= (1<<2);             // P0.2 high = LED OFF
            if (toggle)
            {
                toggle = 0;
            }
